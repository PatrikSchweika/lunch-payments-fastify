import type {LunchRecord, LunchRecordCreate} from 'contracts/src/models/lunch-record.ts'
...
const fetchUserLunchRecords = async (userId: User['id']) => {
  return await API_CLIENT.get<LunchRecord>(`api/users/${userId}/lunchRecords`).then(res => res.data)
}

const fetchLunchRecords = async () => {
    return await API_CLIENT.get<LunchRecord>(`api/lunchRecords`).then(res => res.data)
}

export const useUserLunchRecords = (userId: User['id']) => useQuery({
    queryKey: ['users', userId, 'lunch-records'],
    queryFn: () => fetchUserLunchRecords(userId),
})

export const useLunchRecords = () => useQuery({
